...
public class Packet_1_20_R2 implements PacketManager{
    @Override
    public void sendPacket(Player player, List<MapPacket> packets) {
        ...
        for (MapPacket packet : packets){
            PacketPlayOutMap NMSPacket = new PacketPlayOutMap(packet.mapId, (byte) 4, false, icon, new b(packet.sX, packet.sY, packet.fX, packet.fY, packet.color));
            connection.b(NMSPacket);
        }
    }
}
